#!/usr/bin/env bash
#
# kg-save-gate.sh — PreToolUse(Bash) hook for doxa-discord-bot.
...
if [ "$tool_name" = "mcp__github__merge_pull_request" ]; then
  ...
  [ "$mcp_repo" = "doxa-discord-bot" ] || exit 0   # gate only this repo; siblings pass
...
# doxa-discord-bot landings ONLY — it must never block a sibling repo's push.
