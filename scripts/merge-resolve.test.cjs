...
// (ported from doxa-cns: check-kg-merge-integrity.test.cjs only tests the
// DETECTOR; this covers the RESOLVER — the tool an operator actually runs to
// fix a flagged conflict, and the one that WRITES state).
//
...
//
// Run: node --test scripts/merge-resolve.test.cjs
...
test('is deterministic regardless of (ours, theirs) argument order, including same-timestamp ties', () => {
  const root = sandbox();
  // Both observations share a timestamp — the case a timestamp-only sort
  // could fail to keep stable across argument order.
