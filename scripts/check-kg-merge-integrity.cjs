 ...
 * data-loss class (ported from doxa-cns, Garth 2026-08-13: "this should be
 * standing doctrine and practice across all repos").
 *
 * .cjs extension because this repo's package.json sets "type": "module" —
 * a plain .js file would be loaded as an ES module and require() below
 * would throw.
 *
 * Real incident (doxa-cns PR #343): a real bug shipped fine, but its KG
 * entity was silently lost when a merge conflict on
 * .knowledge-graph-merkle.json was resolved with `git checkout --theirs` —
 * the branch's own observation was discarded with no error anywhere,
 * because the observations array is an APPEND LOG and picking one side can
 * only ever keep a subset. See mergeResolve() in knowledge-graph-merkle.cjs
 * for the correct fix (hash-keyed union of both sides).
 *
 ...
 *   fail-open philosophy of every other gate in this repo). ALSO exit 0 on
 *   any unexpected internal error (a crash must never read as a violation —
 *   see the top-level try/catch in main()'s caller below: an uncaught throw
 *   here must never exit 1, indistinguishable from a confirmed drop, since
 *   the calling hook blocks on exit 1 — a bug in THIS script must never
 *   wrongly block a legitimate landing instead of failing open like every
 *   other check here).
...
// maxBuffer: execSync's 1MB default silently throws ENOBUFS on a repo whose
// merkle log has grown large — caught by the callers' try/catch and treated
// as "unreadable, fail open," which would make this whole detector a no-op
// on the exact repo it matters most for. 64MB should outlive this repo's KG
// log for a long time; if it doesn't, the failure mode is still fail-open,
// never a false block.
function sh(cmd) {
...
function observationsAt(ref, path) {
  ...
  for (const obs of data.observations) {
    ...
    // indexed under a shared falsy/undefined key — a naive `if (obs &&
    // obs.hash)` would let a non-string hash (e.g. a hand-edited file with a
    // numeric or duplicate-empty hash) collapse multiple observations onto
    // one Map key, silently discarding the others INSIDE the very tool
    // meant to catch silent discarding.
...
function run(base, head) {
  ...
  for (const mergeSha of mergeCommits) {
    ...
    // --name-only pre-check here would silently skip every real merge
    // commit and make this whole detector a no-op. Reading the observation
    // sets at merge+both parents unconditionally is cheap enough not to
    // need the filter.
...
// violation" apart from "this script crashed" by exit code alone (both would
// otherwise be exit 1), and a crash must never block a legitimate landing.
try {
  main();
} catch (err) {
